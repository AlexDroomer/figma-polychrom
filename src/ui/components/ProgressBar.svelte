<script lang="ts">
  ...
  const conclusionArray = Array.from({ length: conclusionScores.length }).map(
    (_, i) => {
...
<div class="flex items-center justify-center">
  <div class="flex items-center">
    ...
    <div
      style:background-color="var({ThemeVariablesKeys.secondary12})"
      style:height
      style:width={barWidth}
      class="relative rounded-full"
    >
      <div
        style:background-image="linear-gradient(to right, var({ThemeVariablesKeys.fg70})
        70%, var({ThemeVariablesKeys.fg}) 85%)"
        style:height
        style:width={filledSegmentWidth}
        class="rounded-full"
      >
