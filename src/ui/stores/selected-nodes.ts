...
import { blendColors, type ContrastConclusionList } from '~ui/services/blend/blend-colors.ts';
...
export const isP3 = computed(userSelection, (selection) => 'colorSpace' in selection ? selection.colorSpace === 'DISPLAY_P3' : false);

export const isMultiSelection = computed(userSelection, (selection) => 'selectedNodePairs' in selection ? selection.selectedNodePairs.length > 1 : false);

export const isSingleSelection = computed(userSelection, (selection) => 'selectedNodePairs' in selection ? selection.selectedNodePairs.length === 1 : false);

export const isInvalidBackground = computed(userSelection, (selection) => 'text' in selection && selection.text === SelectionMessageTypes.invalidBackground);

export const isUnprocessedBlendModes = computed(userSelection, (selection) => 'text' in selection && selection.text === SelectionMessageTypes.unprocessedBlendModes);
...
onMount(userSelection, () => {
	const addMessageListener = (event: MessageEvent<Message<SelectionChangeEvent>>): void => {
...
onSet(userSelection, ({ newValue }) => {
	const start = async (): Promise<void> => {
		if ('selectedNodePairs' in newValue) {
			const res = await blendColors(newValue.selectedNodePairs, newValue.colorSpace);
...
const setRewardAnimationLaunch = (contrastConclusionList: ContrastConclusionList): void => {
